...
def simulate_multiple(epoches, case, infectivity=0.02):
    total_sum = np.array([])
    ...
    for k in range(epoches):
        total, current, time, population = simulate(case=case,
                                                ...
                                                total_neighbors=4,
                                                gathering_size=100, gathering_probability=0.5,
...
def main():
    ...
    infectivity_array = [0.02, 0.05, 0.1, 0.2, 0.5, 1]
    for value in infectivity_array:
        simulate_multiple(epoches, NO_GATHERING, value)

    #simulate_multiple(epoches, NO_GATHERING)
    #simulate_multiple(epoches, SINGLE_REPEATED)
